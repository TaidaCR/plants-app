...
export default function CustomDatePicker({text, name, selected, handleOnChange, placeholderText, className, required = false}){
    return(
        <label className={`pb-[10px] bg-white p-3 rounded-xl flex justify-between font-normal text-detail ${className || ""}`}>
            ...
            <DatePicker className="bg-secondary" required={required} name={name} placeholderText={placeholderText} dateFormat="dd/MM/yyyy" selected={selected} onChange={handleOnChange}/>
        </label>
    )
}